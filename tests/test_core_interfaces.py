...
from core.interfaces import (
    PatternConfig, PatternSignal, PatternType, 
    EnhancedPatternDetector, BaseDetector, DataValidator, PatternEngine
)
...
class TestDataValidator:
    ...
    def test_price_logic_validation(self, corrupted_dataframe):
        ...
        with pytest.raises(ValueError, match="High prices cannot be lower than low prices"):
...
class TestEnhancedPatternDetector:
    ...
    def test_validate_data_insufficient_length(self):
        ...
        detector = TestEnhancedPatternDetector.MockDetector(PatternConfig())
        short_data = pd.DataFrame({'open': [1], 'high': [2], 'low': [0.5], 'close': [1.5], 'volume': [1000]})
    ...
    def test_validate_data_missing_columns(self):
        ...
        detector = TestEnhancedPatternDetector.MockDetector(PatternConfig())
        incomplete_data = pd.DataFrame({'open': [1], 'high': [2], 'low': [0.5], 'close': [1.5]})
