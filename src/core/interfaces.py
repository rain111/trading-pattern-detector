...
class EnhancedPatternDetector(ABC):
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            for col in ['open', 'high', 'low', 'close', 'volume']:
                if col in data.columns:
                    ...
                    if col in ['open', 'high', 'low', 'close']:
                        if (data[col] <= 0).any():
                            self.logger.error(f"Column '{col}' contains non-positive values")
                            return False
            ...
            self.logger.error(f"Unexpected error during data validation: {e}", exc_info=True)
            return False
...
class PatternEngine:
    ...
    def __init__(self, detectors: List[EnhancedPatternDetector], config: Optional[Dict[str, Any]] = None):
    ...
    def detect_patterns(self, data: pd.DataFrame, symbol: str) -> List[PatternSignal]:
        ...
        try:
            ...
            self.logger.info(f"Starting pattern detection for {symbol} with {len(self.detectors)} detectors")
            all_signals = []
            ...
            for i, detector in enumerate(self.detectors):
                try:
                    ...
                    self.logger.debug(f"Running detector {i+1}/{len(self.detectors)}: {detector_name}")
                    
                    # Validate data for this detector
                    if not detector.validate_data(data):
                        self.logger.warning(f"Data validation failed for detector {detector_name}")
                        continue
                    ...
                    if signals:
                        ...
                        for signal in signals:
                            try:
                                ...
                                if (signal.entry_price > 0 and signal.stop_loss > 0 and 
                                    signal.target_price > 0 and signal.confidence > 0):
                                    valid_signals.append(signal)
                                else:
                        ...
                        self.logger.debug(f"Detector {detector_name} found {len(valid_signals)} signals")
                    else:
                        self.logger.debug(f"Detector {detector_name} found no signals")
                        
                except Exception as e:
                    self.logger.error(f"Error in detector {detector.__class__.__name__}: {e}", exc_info=True)
                    continue
            ...
            self.logger.error(f"Critical error during pattern detection for {symbol}: {e}", exc_info=True)
            return []
