...
class VolumeAnalyzer:
    ...
    def analyze_volume_patterns(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'volume_std': data['volume'].rolling(window=self.volume_window).std().iloc[-1] if len(data) > 0 else 0
            }
    ...
    def detect_volume_surge(self, data: pd.DataFrame, 
                           threshold: float = 2.0, 
                           lookback: int = 5) -> bool:
    ...
    def calculate_money_flow_index(self, data: pd.DataFrame, 
    ...
    def calculate_volume_weighted_average_price(self, data: pd.DataFrame, 
    ...
    def detect_volume_divergence(self, data: pd.DataFrame, 
        ...
        try:
            ...
            return {
                ...
                'price_change_pct': (price_change / data['close'].iloc[-lookback]) * 100,
                'volume_change_pct': (volume_change / data['volume'].iloc[-lookback]) * 100
            }
        except Exception as e:
