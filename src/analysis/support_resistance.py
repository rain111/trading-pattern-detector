...
class SupportResistanceDetector:
    ...
    def find_support(self, data: pd.DataFrame, tolerance: float = None) -> List[Dict[str, Any]]:
        """Find support levels"""
        try:
            ...
            for i in range(1, len(lows) - 1):
                if (lows.iloc[i] < lows.iloc[i-1] and lows.iloc[i] < lows.iloc[i+1] and
                    ...
                    test_count = self._count_level_tests(data, lows.iloc[i], is_support=True)
                    ...
                    support_levels.append({
                        ...
                        'strength': self._calculate_level_strength(data, lows.iloc[i], is_support=True),
    ...
    def find_resistance(self, data: pd.DataFrame, tolerance: float = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(1, len(highs) - 1):
                if (highs.iloc[i] > highs.iloc[i-1] and highs.iloc[i] > highs.iloc[i+1] and
                    ...
                    # Check how many times this level has been tested
                    test_count = self._count_level_tests(data, highs.iloc[i], is_support=False)
                    ...
                    resistance_levels.append({
                        ...
                        'timestamp': data.index[i],
                        'test_count': test_count,
                        'strength': self._calculate_level_strength(data, highs.iloc[i], is_support=False),
    ...
    def find_dynamic_support_resistance(self, data: pd.DataFrame, 
        ...
        try:
            ...
            return {
                ...
                'support_resistance_zone': self._calculate_sr_zone(dynamic_support, dynamic_resistance)
            }
        except Exception as e:
            self.logger.error(f"Error finding dynamic support/resistance: {e}")
            return {}
    
    def draw_trendlines(self, data: pd.DataFrame, swing_points: Dict[str, List]) -> List[Dict]:
        ...
        try:
            ...
            self._add_trendline_type(trendlines, swing_lows, 'uptrend', min_points, max_distance)
            
            # Downtrend lines (connecting lower highs)
            self._add_trendline_type(trendlines, swing_highs, 'downtrend', min_points, max_distance)
            ...
            for level in significant_levels:
                trendlines.append({
                    ...
                    'is_broken': self._is_level_broken(data, level['price'], level['is_support'])
                })
    ...
    def get_current_market_context(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Find nearest levels
            nearest_support = self._find_nearest_level(current_price, support_levels, is_support=True)
            nearest_resistance = self._find_nearest_level(current_price, resistance_levels, is_support=False)
            
            # Calculate distance to levels
            support_distance = (current_price - nearest_support['price']) / nearest_support['price'] * 100 if nearest_support else float('inf')
            resistance_distance = (nearest_resistance['price'] - current_price) / nearest_resistance['price'] * 100 if nearest_resistance else float('inf')
            
            # Determine market position
            market_position = self._determine_market_position(current_price, support_levels, resistance_levels)
            ...
            return {
                ...
                'support_resistance_ratio': resistance_distance / support_distance if support_distance != 0 and resistance_distance != 0 else 1.0,
                'key_levels': {
                    'strong_support': sorted([s for s in support_levels if s['strength'] > 0.7], 
                                           key=lambda x: x['strength'], reverse=True)[:3],
                    'strong_resistance': sorted([r for r in resistance_levels if r['strength'] > 0.7], 
                                             key=lambda x: x['strength'], reverse=True)[:3]
                }
            }
        except Exception as e:
    ...
    def _count_level_tests(self, data: pd.DataFrame, level_price: float, is_support: bool) -> int:
    ...
    def _calculate_level_strength(self, data: pd.DataFrame, level_price: float, is_support: bool) -> float:
    ...
    def _create_price_zone(self, level_price: float, tolerance: float) -> Dict[str, float]:
    ...
    def _combine_nearby_levels(self, levels: List[Dict], distance_threshold: float = 0.01) -> List[Dict]:
    ...
    def _add_trendline_type(self, trendlines: List[Dict], points: List[str], 
                          trend_type: str, min_points: int, max_distance: int):
    ...
    def _find_significant_levels(self, swing_highs: List[Dict], 
        ...
        for high in swing_highs:
            if high['strength'] > 0.5:
                all_levels.append({'price': high['price'], 'is_support': False, 'strength': high['strength']})
        
        for low in swing_lows:
            if low['strength'] > 0.5:
                all_levels.append({'price': low['price'], 'is_support': True, 'strength': low['strength']})
        
        return sorted(all_levels, key=lambda x: x['strength'], reverse=True)
    
    def _find_nearest_level(self, price: float, levels: List[Dict], is_support: bool) -> Dict:
        """Find the nearest support/resistance level"""
        if not levels:
            return {}
        
        # Filter by type
        filtered_levels = [level for level in levels if level.get('is_support', None) == is_support]
    ...
    def _determine_market_position(self, price: float, support_levels: List[Dict], 
        ...
        try:
            ...
            nearest_support = self._find_nearest_level(price, support_levels, is_support=True)
            nearest_resistance = self._find_nearest_level(price, resistance_levels, is_support=False)
            
            support_distance = (price - nearest_support['price']) / nearest_support['price'] * 100
            resistance_distance = (nearest_resistance['price'] - price) / nearest_resistance['price'] * 100
    ...
    def _is_level_broken(self, data: pd.DataFrame, level_price: float, is_support: bool) -> bool:
    ...
    def _calculate_sr_zone(self, support: pd.Series, resistance: pd.Series) -> pd.Series:
