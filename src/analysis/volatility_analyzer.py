...
class VolatilityAnalyzer:
    ...
    def calculate_volatility_contraction(
        ...
        try:
            ...
            # Calculate ATR trend
            atr_trend = np.polyfit(
                range(len(atr_series[-period:])), atr_series[-period:].values, 1
            )[0]
            ...
            contraction_ratio = (
                recent_atr / historical_atr if historical_atr > 0 else 1.0
            )
    ...
    def calculate_keltner_channels(
        self, data: pd.DataFrame, period: int = 20, atr_multiplier: float = 2
    ) -> Dict[str, pd.Series]:
