...
class TrianglePatternDetector(BaseDetector):
    ...
    def _analyze_triangle_pattern(
        ...
        try:
            ...
            if ascending_triangle["is_triangle"]:
                signals.append(
                    self._generate_triangle_signal(
                        data,
                        start_idx,
                        ascending_triangle,
    ...
    def _detect_ascending_triangle(
        ...
        try:
            ...
            lows = triangle_data[
                triangle_data["low"]
                == triangle_data["low"].rolling(window=3, center=True).min()
            ]
            ...
            resistance_std = triangle_data[triangle_data["high"] >= resistance * 0.98][
                "high"
            ].std()
            resistance_horizontal = resistance_std < resistance * 0.02

            # Check for breakout
            breakout_info = self._check_triangle_breakout(
                data, start_idx, triangle_length, resistance, support_slope > 0
            )
    ...
    def _detect_descending_triangle(
        ...
        try:
            ...
            highs = triangle_data[
                triangle_data["high"]
                == triangle_data["high"].rolling(window=3, center=True).max()
            ]
            ...
            support_std = triangle_data[triangle_data["low"] <= support * 1.02][
                "low"
            ].std()
            ...
            # Check for breakout
            breakout_info = self._check_triangle_breakout(
                data, start_idx, triangle_length, support, resistance_slope < 0
            )
    ...
    def _detect_symmetrical_triangle(
        ...
        try:
            ...
            converging = (
                abs(resistance_slope - support_slope) < abs(support_slope) * 0.3
            )
            ...
            # Check for breakout
            breakout_info = self._check_triangle_breakout(
                data, start_idx, triangle_length, None, support_slope > resistance_slope
            )
        ...
        try:
            ...
            if is_bullish and level_price:
                ...
                breakout_price = breakout_data[
                    breakout_data["high"] > level_price * 1.01
                ].iloc[0]["high"]
            else:
                # Bearish breakout below support
                breakout_confirmed = breakout_data["low"].min() < level_price * 0.99
                breakout_price = breakout_data[
                    breakout_data["low"] < level_price * 0.99
                ].iloc[0]["low"]

            if breakout_confirmed:
                volume_spike = (
                    breakout_data["volume"].mean()
                    > data["volume"].iloc[:breakout_start].mean() * 1.5
                )
                ...
                return {
                    ...
                    "breakout_strength": (
                        abs(breakout_price - level_price) / level_price
                        if level_price
                        else 0.02
                    ),
                }
        ...
        try:
            ...
            if is_bullish:
                ...
                risk_distance = (
                    breakout_price
                    - data["low"]
                    .iloc[start_idx : start_idx + triangle_info["triangle_length"]]
                    .min()
                )
            ...
            pattern_data = {
                ...
                "pattern_formation": self._calculate_formation_quality(
                    triangle_info, data, start_idx
                ),
            }
    ...
    def _calculate_formation_quality(
        self, triangle_info: dict, data: pd.DataFrame, start_idx: int
    ) -> float:
        ...
        try:
            ...
            length_score = (
                1
                - abs(triangle_info["triangle_length"] - optimal_length)
                / optimal_length
            )
            quality += length_score * 0.2

            # Volume quality
            volume_score = self._check_volume_triangle_pattern(
                data.iloc[start_idx : start_idx + triangle_info["triangle_length"]]
            )
            if volume_score:
                quality += 0.2
            ...
            if triangle_info["triangle_type"] == "symmetrical":
                slope_diff = abs(
                    triangle_info["resistance_slope"] - triangle_info["support_slope"]
                )
    ...
    def _calculate_triangle_confidence(
        self, triangle_info: dict, breakout_info: dict
    ) -> float:
