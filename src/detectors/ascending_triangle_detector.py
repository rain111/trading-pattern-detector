...
class AscendingTriangleDetector(BaseDetector):
    ...
    def _analyze_ascending_triangle(self, data: pd.DataFrame, end_idx: int) -> List[PatternSignal]:
        ...
        try:
            ...
            if pattern_info:
                signal = self._create_ascending_triangle_signal(pattern_info, data, end_idx)
                if signal:
    ...
    def _identify_ascending_triangle(self, data: pd.DataFrame, start_idx: int, end_idx: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            ascending_trendline = self._find_ascending_trendline(window_data, horizontal_resistance['price'])
            if not ascending_trendline:
                return None

            # Check if both lines intersect properly
            if not self._check_triangle_formation(horizontal_resistance, ascending_trendline, window_data):
                return None

            # Check for breakout above horizontal resistance
            current_price = data.iloc[end_idx]['close']
            if current_price > horizontal_resistance['price'] * (1 + self.breakout_threshold):
                return {
    ...
    def _find_horizontal_resistance(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            resistance_peaks = [p for p in peaks if abs(p['price'] - mean_price) / mean_price <= self.max_horizontal_deviation]
    ...
    def _find_ascending_trendline(self, data: pd.DataFrame, resistance_price: float) -> Optional[Dict[str, Any]]:
        """Find ascending trendline"""
        try:
    ...
    def _check_triangle_formation(self, horizontal_resistance: Dict[str, Any], ascending_trendline: Dict[str, Any], data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            trendline_end_price = ascending_trendline['slope'] * (len(data) - 1) + ascending_trendline['intercept']
            convergence_ratio = abs(trendline_end_price - resistance_price) / resistance_price
    ...
    def _create_ascending_triangle_signal(self, pattern_info: Dict[str, Any], data: pd.DataFrame, end_idx: int) -> Optional[PatternSignal]:
        ...
        try:
            ...
            confidence = self._calculate_ascending_triangle_confidence(pattern_info, data, end_idx)
    ...
    def _calculate_ascending_triangle_confidence(self, pattern_info: Dict[str, Any], data: pd.DataFrame, end_idx: int) -> float:
