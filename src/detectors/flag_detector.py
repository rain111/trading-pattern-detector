...
class FlagPatternDetector(BaseDetector):
    ...
    def _is_potential_flag_start(self, data: pd.DataFrame, start_idx: int) -> bool:
        ...
        try:
            ...
            move = (
                move_data["close"].iloc[-1] - move_data["close"].iloc[0]
            ) / move_data["close"].iloc[0]
    ...
    def _detect_flag_structure(
        ...
        try:
            ...
            flagpole_height = abs(
                (flagpole_data["close"].iloc[-1] - flagpole_data["close"].iloc[0])
                / flagpole_data["close"].iloc[0]
            )
            ...
            overall_direction = (
                "bullish"
                if flagpole_data["close"].iloc[-1] > flagpole_data["close"].iloc[0]
                else "bearish"
            )
    ...
    def _generate_flag_signal(
        self, data: pd.DataFrame, start_idx: int, flag_info: dict
    ) -> PatternSignal:
        ...
        try:
            ...
            if flag_info["overall_direction"] == "bullish":
                ...
                target_price = (
                    breakout_price + target_distance * self.config.reward_ratio
                )
            ...
            pattern_data = {
                ...
                "flagpole_volume": data["volume"]
                .iloc[start_idx : start_idx + 15]
                .mean(),
                "flag_volume": data["volume"].iloc[flag_end - 15 : flag_end].mean(),
                "breakout_strength": abs(current_price - breakout_price)
                / breakout_price,
            }
    ...
    def _calculate_signal_strength(self, pattern_data: dict) -> float:
        ...
        try:
            ...
            volume_ratio = pattern_data["flag_volume"] / pattern_data.get(
                "flagpole_volume", 1
            )
            ...
            duration_score = min(
                pattern_data["flag_duration"] / 10, 1.0
            )  # Optimal duration 10 periods
