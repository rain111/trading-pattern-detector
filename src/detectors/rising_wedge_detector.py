...
class RisingWedgeDetector(BaseDetector):
    """Detector for Rising Wedge patterns"""

    def __init__(self, config: PatternConfig):
        super().__init__(config)
        self.logger = logging.getLogger(self.__class__.__name__)

        # Rising Wedge parameters
        ...
        self.min_volume_spike = 1.1  # Minimum volume spike for breakdown
    ...
    def detect_pattern(self, data: pd.DataFrame) -> List[PatternSignal]:
        """Detect Rising Wedge patterns in the data"""
        signals = []
        ...
        try:
            # Look for potential rising wedge patterns
            for i in range(self.min_pattern_length, len(data)):
                pattern_signals = self._analyze_rising_wedge(data, i)
                signals.extend(pattern_signals)

        except Exception as e:
            self.logger.error(f"Error in rising wedge detection: {e}")

        return signals

    def _analyze_rising_wedge(self, data: pd.DataFrame, end_idx: int) -> List[PatternSignal]:
        """Analyze potential rising wedge pattern ending at end_idx"""
        signals = []
        ...
        try:
            ...
            # Check for rising wedge formation
            pattern_info = self._identify_rising_wedge(data, start_idx, end_idx)

            if pattern_info:
                signal = self._create_rising_wedge_signal(pattern_info, data, end_idx)
                if signal:
                    signals.append(signal)

        except Exception as e:
            self.logger.debug(f"Error analyzing rising wedge: {e}")

        return signals

    def _identify_rising_wedge(self, data: pd.DataFrame, start_idx: int, end_idx: int) -> Optional[Dict[str, Any]]:
        """Identify rising wedge pattern"""
        try:
            window_data = data.iloc[start_idx:end_idx]

            # Find upper trendline (connecting higher highs)
            upper_trendline = self._find_upper_trendline(window_data)
            if not upper_trendline:
                return None

            # Find lower trendline (connecting higher lows)
            lower_trendline = self._find_lower_trendline(window_data)
            if not lower_trendline:
                return None

            # Check if both lines form a rising wedge
            ...
            trendline_price_at_end = self._calculate_trendline_price(lower_trendline, end_idx - start_idx)

            if current_price < trendline_price_at_end * (1 - self.breakdown_threshold):
                return {
            ...
            self.logger.debug(f"Error identifying rising wedge: {e}")
            return None
    ...
    def _check_wedge_formation(self, upper_trendline: Dict[str, Any], lower_trendline: Dict[str, Any], data: pd.DataFrame) -> bool:
        """Check if both lines form a valid rising wedge"""
        try:
            # Check if both trendlines are ascending
            if upper_trendline['slope'] <= 0 or lower_trendline['slope'] <= 0:
                return False

            # Check if upper trendline is steeper than lower trendline
    ...
    def _create_rising_wedge_signal(self, pattern_info: Dict[str, Any], data: pd.DataFrame, end_idx: int) -> Optional[PatternSignal]:
        """Create trading signal for rising wedge pattern"""
        try:
            # Entry price at breakdown below lower trendline
            entry_price = data.iloc[end_idx]['close']
            trendline_price = self._calculate_trendline_price(pattern_info['lower_trendline'], end_idx - pattern_info['pattern_start_idx'])

            # Stop loss above the upper trendline
            upper_trendline_price = self._calculate_trendline_price(pattern_info['upper_trendline'], end_idx - pattern_info['pattern_start_idx'])
            stop_loss = upper_trendline_price * 1.02  # 2% buffer above upper trendline

            # Target price based on wedge height
            wedge_height = upper_trendline_price - trendline_price
            target_price = trendline_price - wedge_height * 0.5  # Conservative target

            # Calculate confidence
            confidence = self._calculate_rising_wedge_confidence(pattern_info, data, end_idx)
            ...
            self.logger.error(f"Error creating rising wedge signal: {e}")
            return None

    def _calculate_rising_wedge_confidence(self, pattern_info: Dict[str, Any], data: pd.DataFrame, end_idx: int) -> float:
        """Calculate confidence score for rising wedge pattern"""
