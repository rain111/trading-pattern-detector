...
class StageAnalyzer:
    ...
    def detect_volatility_contraction(self, data: pd.DataFrame, start_idx: int, 
                                    period: int = 30) -> bool:
    ...
    def detect_consolidation(self, data: pd.DataFrame, start_idx: int, 
                           max_range: float = 0.05) -> Dict[str, Any]:
        ...
        try:
            ...
            high_range = consolidation_data['high'].max() - consolidation_data['high'].min()
            low_range = consolidation_data['low'].max() - consolidation_data['low'].min()
            price_range = consolidation_data['close'].max() - consolidation_data['close'].min()
    ...
    def detect_breakout(self, data: pd.DataFrame, consolidation_info: Dict,
                       breakout_threshold: float = 0.02) -> Dict[str, Any]:
        """Detect breakout stage"""
        try:
            consolidation_start = consolidation_info.get('start_idx', 0)
            consolidation_end = consolidation_start + consolidation_info.get('consolidation_length', 0)
            ...
            breakout_confirmed = breakout_data['high'].max() > resistance * (1 + breakout_threshold)
            
            if breakout_confirmed:
                breakout_price = breakout_data[breakout_data['high'] > resistance * (1 + breakout_threshold)].iloc[0]['high']
                ...
                return {
                    ...
                    'confidence': self._calculate_breakout_confidence(breakout_data, consolidation_info)
                }
            else:
                return {'breakout_confirmed': False}
        except Exception as e:
    ...
    def _calculate_breakout_confidence(self, breakout_data: pd.DataFrame, 
                                    consolidation_info: Dict) -> float:
        """Calculate breakout confidence"""
        try:
            confidence = 0.5
            
            # Volume confidence
            volume_ratio = breakout_data['volume'].mean() / consolidation_info.get('avg_volume', 1)
            confidence += min(volume_ratio * 0.2, 0.2)
            
            # Price momentum confidence
            momentum = (breakout_data['close'].iloc[-1] - breakout_data['close'].iloc[0]) / breakout_data['close'].iloc[0]
...
class VCPBreakoutDetector(BaseDetector):
    ...
    def _analyze_vcp_pattern(self, data: pd.DataFrame, start_idx: int) -> List[PatternSignal]:
        ...
        try:
            ...
            consolidation_info = self._detect_consolidation_with_support_resistance(data, start_idx)
    ...
    def _detect_consolidation_with_support_resistance(self, data: pd.DataFrame, start_idx: int) -> Dict[str, Any]:
        ...
        try:
            ...
            resistance_touches = (consolidation_data['high'] >= resistance * 0.995).sum()
            
            return {
                'is_consolidation': range_ratio < 0.05 and support_touches >= 2 and resistance_touches >= 2,
                'price_range': price_range,
    ...
    def _generate_vcp_signal(self, data: pd.DataFrame, start_idx: int,
        ...
        try:
            ...
            stop_loss = min(consolidation_info['support'] * 0.98, breakout_price * 0.995)
            ...
            pattern_data = {
                ...
                'volatility_contraction': self._calculate_volatility_contraction_score(data, start_idx)
            }
    ...
    def _calculate_volatility_contraction_score(self, data: pd.DataFrame, start_idx: int) -> float:
        ...
        try:
            ...
            consolidation_data = data.iloc[consolidation_start:consolidation_start + consolidation_period]
    ...
    def _calculate_signal_strength(self, pattern_data: dict) -> float:
        ...
        try:
            ...
            touches = pattern_data.get('support_touches', 0) + pattern_data.get('resistance_touches', 0)
