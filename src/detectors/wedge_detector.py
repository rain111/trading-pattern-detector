...
class WedgePatternDetector(BaseDetector):
    ...
    def _analyze_wedge_pattern(
        ...
        try:
            ...
            if rising_wedge["is_wedge"]:
                signals.append(
                    self._generate_wedge_signal(
                        data, start_idx, rising_wedge, PatternType.WEDGE_PATTERN
                    )
                )
            ...
            if falling_wedge["is_wedge"]:
                signals.append(
                    self._generate_wedge_signal(
                        data, start_idx, falling_wedge, PatternType.WEDGE_PATTERN
                    )
                )
    ...
    def _detect_rising_wedge(
        ...
        try:
            ...
            resistance_slope = np.polyfit(
                range(len(swing_highs)), swing_highs["high"].values, 1
            )[0]
            support_slope = np.polyfit(
                range(len(swing_lows)), swing_lows["low"].values, 1
            )[0]
            ...
            rising_wedge = (
                resistance_slope > 0
                and support_slope > 0
                and resistance_slope > support_slope
                and abs(resistance_slope - support_slope) > 0.001
            )
            ...
            # Check for breakout
            breakout_info = self._check_wedge_breakout(
                data, start_idx, wedge_length, resistance_slope > support_slope
            )
    ...
    def _detect_falling_wedge(
        ...
        try:
            ...
            falling_wedge = (
                resistance_slope < 0
                and support_slope < 0
                and support_slope < resistance_slope
                and abs(resistance_slope - support_slope) > 0.001
            )
            ...
            # Check for breakout
            breakout_info = self._check_wedge_breakout(
                data, start_idx, wedge_length, support_slope < resistance_slope
            )
        ...
        try:
            ...
            if is_bullish_breakout:
                ...
                if len(resistance_highs) > 0:
                    resistance_trend = np.polyfit(
                        range(len(resistance_highs)), resistance_highs.values, 1
                    )[0]
                ...
                if len(support_lows) > 0:
                    support_trend = np.polyfit(
                        range(len(support_lows)), support_lows.values, 1
                    )[0]
            ...
            if breakout_confirmed:
                ...
                return {
                    ...
                    "breakout_strength": abs(
                        breakout_price - data["close"].iloc[breakout_start]
                    )
                    / data["close"].iloc[breakout_start],
                }
            else:
                return {"breakout_confirmed": False}
        except Exception as e:
        ...
        try:
            ...
            pattern_data = {
                ...
                "pattern_formation": self._calculate_formation_quality(
                    wedge_info, data, start_idx
                ),
                "wedge_angle": abs(
                    wedge_info["resistance_slope"] - wedge_info["support_slope"]
                ),
            }
    ...
    def _calculate_formation_quality(
        self, wedge_info: dict, data: pd.DataFrame, start_idx: int
    ) -> float:
        ...
        try:
            ...
            optimal_length = 50
            length_score = (
                1 - abs(wedge_info["wedge_length"] - optimal_length) / optimal_length
            )
            ...
            swing_quality = (
                min(
                    len(wedge_info["swing_highs"]) + len(wedge_info["swing_lows"]) - 4,
                    1.0,
                )
                * 0.1
            )
    ...
    def _calculate_wedge_confidence(
        self, wedge_info: dict, breakout_info: dict
    ) -> float:
