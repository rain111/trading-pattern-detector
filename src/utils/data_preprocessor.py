...
class DataPreprocessor:
    ...
    def align_data_timestamps(self, data: pd.DataFrame, timezone: str = 'UTC') -> pd.DataFrame:
    ...
    def _calculate_bollinger_bands(self, prices: pd.Series, period: int = 20, std_dev: float = 2) -> pd.DataFrame:
    ...
    def create_synthetic_market_data(self, start_date: str, end_date: str, 
        ...
        try:
            ...
            data['high'] = data[['open', 'close']].max(axis=1) * (1 + daily_ranges.abs() / 2)
            data['low'] = data[['open', 'close']].min(axis=1) * (1 - daily_ranges.abs() / 2)
