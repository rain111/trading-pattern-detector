...
class SignalAggregator:
    ...
    def rank_signals(self, ranking_method: str = "confidence") -> List[PatternSignal]:
        ...
        if ranking_method == "confidence":
            ...
            return sorted(
                self.signals, key=lambda x: x.target_price / x.stop_loss, reverse=True
            )
            ...
            return sorted(
                self.signals, key=lambda x: x.probability_target or 0.5, reverse=True
            )
        elif ranking_method == "reward_ratio":
            reward_ratios = []
            for signal in self.signals:
                reward_ratio = (signal.target_price - signal.entry_price) / (
    ...
    def get_signals_by_pattern_type(
        self, pattern_type: PatternType
    ) -> List[PatternSignal]:
        """Get signals by pattern type"""
        return [
            signal for signal in self.signals if signal.pattern_type == pattern_type
        ]
    ...
    def remove_duplicate_signals(self, tolerance: float = 0.02) -> List[PatternSignal]:
        ...
        for i, signal1 in enumerate(self.signals):
            ...
            for j, signal2 in enumerate(processed_signals):
                if (
                    signal1.symbol == signal2.symbol
                    and signal1.pattern_type == signal2.pattern_type
                ):
                    # Check if signals are too similar
                    price_diff = (
                        abs(signal1.entry_price - signal2.entry_price)
                        / signal2.entry_price
                    )
    ...
    def get_top_signals(
        self, n: int = 10, ranking_method: str = "confidence"
    ) -> List[PatternSignal]:
    ...
    def get_signals_by_time_period(
    ...
    def calculate_portfolio_risk_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_signals = len(self.signals)
            recommended_position_size = 1.0 / max(
                total_signals, 1
