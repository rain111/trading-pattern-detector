    ...
    def _load_configurations(self):
        ...
        try:
            ...
            pattern_config_path = os.path.join(
                self.config_dir, "pattern_parameters.yaml"
            )
            if os.path.exists(pattern_config_path):
                self.pattern_params = PatternParameters.load_from_yaml(
                    pattern_config_path
                )

            # Load market data configuration
            market_data_config_path = os.path.join(
                self.config_dir, "market_data_config.yaml"
            )
            if os.path.exists(market_data_config_path):
                self.market_data_config = MarketDataConfig.load_from_yaml(
                    market_data_config_path
                )

            # Load detection settings
            detection_settings_path = os.path.join(
                self.config_dir, "detection_settings.yaml"
            )
            if os.path.exists(detection_settings_path):
                self.detection_settings = DetectionSettings.load_from_yaml(
                    detection_settings_path
                )
    ...
    def import_configuration(self, input_path: str) -> None:
        ...
        try:
            ...
            if "pattern_parameters" in config_data:
                self.pattern_params = PatternParameters.from_dict(
                    config_data["pattern_parameters"]
                )

            if "market_data_config" in config_data:
                self.market_data_config = MarketDataConfig.from_dict(
                    config_data["market_data_config"]
                )

            if "detection_settings" in config_data:
                self.detection_settings = DetectionSettings.from_dict(
                    config_data["detection_settings"]
                )
